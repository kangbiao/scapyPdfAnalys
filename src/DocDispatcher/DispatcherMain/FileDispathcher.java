...
public class FileDispathcher {
	...
	private void pdfToHtml() {
		...
		while((pdf=getFiledata())!=null){
			isDeal=true;
			String htmlholder=pdf.getFolderpath()+File.separator+ServerPublic.HTMLFOLD;
			File folder=new File(htmlholder);
			if(!folder.exists())
				folder.mkdirs();
			String htmlpath=htmlholder+File.separator+ServerPublic.HTMLNAME;
			StringBuilder builder=new StringBuilder();
			builder.append("pdftohtml ")
			   .append(CMDPARAMS).append(pdf.getFilepath()).append(" ")
